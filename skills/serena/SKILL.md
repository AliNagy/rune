...
requires a forbidden path, that is drift — stop and report it (`drift`), do not
...
that is an `init` problem, not something to solve by browsing.
...
- **Serena memories** — stable, agent-facing background knowledge about the codebase.
  ...
  `survey` workers. `init`, `vision`, and `handoff` dispatch those workers;
