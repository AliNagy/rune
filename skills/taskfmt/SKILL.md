...
| `milestones.md` | the one worker on `decompose` assigned the milestone-graph job |
| `sessions/<stamp>.md` | the parent |
| `map.md` | a worker on `survey` |
...
| `tasks/T-nnn.md` | the single reconciling worker on `decompose` that creates it |
| `notes/T-nnn.progress` | the worker holding T-nnn: `bug` during diagnosis, then `execute` |
| `notes/T-nnn.md` | the worker holding T-nnn |
| `notes/T-nnn.sizing.md` | a worker on `size` |
| `notes/T-nnn.verify.md` | a worker on `verify` |
| `notes/T-nnn.landing.md` | a worker on `land` |
...
| `notes/init-commands.md` | a worker on `oracle` |
...
| `findings/open/FND-nnn.md` | the `verify-finding` worker assigned that exact id and staging path |
...
The canonical ledger schema is owned by `ledger`. Schema 2 has one task row containing
...
Only task-bound workers create child `T-nnn/` worktrees. A successful `land` removes
the clean landed child; `bug` may remove its own unreproduced provisional child; and
`drift` removes an assigned unpublished child while quiescing or abandoning work.
`continue` sends a clean, fully merged orphan to `land`'s cleanup mode. No public route
...
The parent must then follow `root` before reading state. `init`, `vision`, `pause`, and
`handoff` use `mode: initialize`; `using-rune`, `work`, and `continue` use `mode: resolve`.
...
user. `root` is the only skill allowed to interpret a legacy root; workers receive only
...
- For a bug, `bug` creates that exact worktree during `diagnosing`, before it writes the
...
- Worktree removal has exactly three worker owners. `bug` may discard its own exact
  provisional checkout when diagnosis does not reproduce or reclassifies. `drift` may
  discard an assigned unpublished checkout for detect, quiesce, confirmed abandon, or the
  mechanically empty recovery case. `land` removes a successfully landed checkout or
...
|---|---|
| map an unfamiliar codebase | `survey` |
| classify a request | `triage` |
| milestone graph, and milestone → tasks | `decompose` |
| run build / test / lint / typecheck | `oracle` |
| execute one task | `execute` (which also loads `serena`, `drift`) |
| verify one finished task | `verify` |
| land one verified task in the main tree | `land` |
| salvage an abandoned task | `recover` |
| answer a question about the code | `investigate` |
| answer a question from outside the repo | `research` |
| reproduce a bug | `bug` |
...
unused one on return. A direct `research` dispatch receives only its `RES-` assignment.
...
parent creates the new reservation and `bug` reproduces the failure in its new worktree
...
|---|---|
| `execute` | `status: done \| drifted \| budget \| blocked \| question` |
| `verify` | `verdict: pass \| fail \| unverified` |
| `land` | `landing: landed \| refused \| conflict \| reverted \| stuck \| not_landed \| cleaned` |
| `recover` | `verdict: salvage \| discard \| partial` |
| `triage` | `type: bug \| feature \| refactor \| investigation` |
| `bug` | `diagnosis: reproduced \| not_reproduced \| reclassified \| blocked` |
| `oracle` | `oracle: passing \| failing \| none` |
| `decompose` | `plan: graph \| drafted \| reconciled \| blocked` |
| `investigate` | `investigation: answered \| blocked` |
| `research` | `research: answered \| blocked` |
| `drift` | `status: drifted \| recorded \| quiesced \| abandoned \| discarded \| refused \| budget \| question` |
| `survey` | `survey: mapped \| amended \| unchanged \| conflict \| blocked` |
| `root` | `migration: none \| completed \| resumed \| blocked` |
| `verify-finding` | `finding: confirmed \| refuted \| inconclusive` |
| `size` | `sizing: pass \| split \| blocked` |

Then whatever else that skill defines.

`execute` has one required conditional interface. `status: blocked` always adds:
...
an executor's outcome, and `execute`'s values are the ones the ledger's state machine
...
An `decompose` planner returning `plan: drafted` also returns exactly one `artifact:`
...
|---|---|---|
| an individual command was executed or unavailable | `passing \| failing \| unavailable` | each stored `rune.yml` `commands.<name>.status` and the transient `oracle` command digest |
| the configured project oracle's verdict or absence | `passing \| failing \| none` | stored `rune.yml` `oracle.status`, `oracle`'s outcome, verifier evidence, and landing evidence |
...
If it stops the task you are on, that is drift. Use `drift` and do not open a finding.
...
subagent following `verify-finding`** with the claim pointer. Fresh matters: the finder
...
immutable artifact written by the assigned `decompose` reconciler. Before dispatch,
...
before dispatching `decompose`. Thus the only pre-migration worker remains the amendment
...
`size`, before it can be executed** — because five files can be a small job or an
enormous one, and the planner holding the whole milestone is the least able to tell which.
A task is registered `unsized` and only a `pass` moves it to `pending`; see `ledger`.
...
`notes/T-nnn.progress`. Ownership follows the task sequentially: `bug` writes the first
diagnosis block while the row is `diagnosing`; `execute` appends execution evidence only
...
For a bug, `bug` creates the check and first red observation during `diagnosing`, before
