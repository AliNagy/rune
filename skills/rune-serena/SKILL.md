...
requires a forbidden path, that is drift — stop and report it (`rune-drift`), do not
...
- **Serena memories** — stable, agent-facing background knowledge about the codebase.
  ...
  `rune-survey` workers. `rune-init`, `rune-vision`, and `rune-handoff` dispatch those workers;
