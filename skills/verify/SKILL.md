...
from the task and require one allowed contract from `taskfmt`. For a completed legacy
...
`drift` exists precisely to prevent this, so a violation means either the rule
...
`taskfmt` without anyone having to think about it. It has a **different sole writer**
