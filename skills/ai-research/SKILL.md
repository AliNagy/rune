...
- **Question** — in a shape that admits an answer, per `ai-investigate` §1.
...
Breadth before depth, exactly as in `ai-survey`. One query formulation finds one
...
behind that decision — recorded by whoever owns that file, per `ai-taskfmt`, not by you.
If it says the plan is wrong, that is `ai-drift`. If the user wants the recommendation
