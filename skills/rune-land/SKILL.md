...
- optional `mode` — defaults to `land`; `drift-observe` is allowed only when the task is
  ...
  is allowed only for a registered orphan found by `rune-continue`
...
`worktree_path`, task id, and exact full branch ref returned by `rune-continue`'s bounded probe.
...
`rune-drift` quiesce. When the commit is already reachable, the ordinary oracle and cleanup
...
**5. Run the project oracle** in the main tree, per `rune-oracle`. Compare against the
...
the worktree `rune-verify` already ran the oracle in, so a re-run cannot say anything new.
...
record what remains; `rune-continue` can remove the orphan later. Otherwise return
`cleanup: complete`.
...
in `rune-taskfmt` without anyone having to think about it. It is **separate from the
