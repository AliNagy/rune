...
`rune-taskfmt`. Every task leaves a check behind that did not exist before.
...
3. **Return** the outcome honestly. You do not write `rune.yml` — the parent does, from
   ...
   set lives in `rune-init` §5; do not invent a second schema for it. What the parent records
   looks like:
...
uses `passing | failing | none`, exactly as stored by `rune-init`:
...
This one is not a dispatch of its own. A worker on **`rune-land`** merges the exact verified commit
...
the known-red baseline, and treat any new failure as a regression. `rune-land` owns what
...
That is why `rune-taskfmt` requires the task's declared verification evidence in the progress
