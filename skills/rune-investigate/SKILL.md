...
Read-only. `rune-serena` applies with full force — this protocol is where exploration
...
- the reserved `RES-nnn` id and paths: load `rune-research`, follow it for that part, and
...
triage them properly — probably into `rune-feature` or `rune-refactor`, with the
