...
7. **Write the missing handoff** to `<main_root>/.rune/notes/T-nnn.md`, in the format
   `taskfmt` specifies, so the next executor gets what it should have had. Its
...
oracle passing before the refactor is reapplied. `verify` correctly refuses either mode
...
fresh report slot and dispatches `drift` in record-only mode from those durable pointers.
