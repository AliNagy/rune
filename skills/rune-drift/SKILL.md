...
- **record-only** receives the same assigned id, staging and final paths, one or more
  ...
  `rune-verify` verdict says the task's evidence or acceptance contract is not checkable, or
...
- **quiesce** receives an existing causal drift pointer, task id, and the ledger's exact
  ...
  and return `status: refused`; a `landing` task must be reconciled by `rune-land` instead.
- **abandon** receives a task id and the ledger's exact absolute `worktree_path` after
  `rune-pause` has confirmed the active worker stopped and the user accepted the loss. Prove
...
- **discard-empty** receives a task id, the exact registered `worktree_path` and task
  branch, plus `main_root`, after `rune-continue` found a dead executor with an empty diff and
...
**2. Write the handoff note** — `<main_root>/.rune/notes/T-nnn.md`, per `rune-taskfmt`.
