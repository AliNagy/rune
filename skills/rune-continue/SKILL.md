...
`rune-root` may run only its own separately bounded migration probe.

### Mutating lifecycle commands

`none` — reconciliation changes parent-owned coordination files only. Cleanup of a proven
landed orphan is dispatched to `rune-land` cleanup mode; unpublished discard is dispatched
to `rune-drift`.

## Coordination-root preflight

Resolve `main_root` once with the bounded probe `git rev-parse --show-toplevel` before
reading state. Then follow `rune-root` with `work: coordination-root`, that absolute root,
and `mode: resolve`; it
...
which is the single most expensive thing you could do here and the reason `rune-recover`
...
re-measuring to `rune-init`, which owns the rule and the probes.

If `rune.yml` has legacy `commands.*.status: ok | fail` (or the old `none found` command
result), route through `rune-init` before task dispatch so its sole writer performs the exact
canonical migration. `rune-continue` may recognize that shape but never rewrites the manifest.
Any other enum is damaged state and stops.

### Validate or migrate the ledger first

Before treating any row as state, apply `rune-ledger`'s schema validation. `schema: 2` must
...
- Any unfinished amended task fails closed. Choose the lowest-id amended row as the
  ...
  exact staging and final paths, then dispatch `rune-drift` in record-only mode with the
...
- If that worker or session dies, keep the predecessor ledger and its pending assignment;
  the next `rune-continue` reuses the same id and paths. Once staging validates, atomically
...
above. For a completed row, include an `rune-taskfmt` mitigation-repair pending assignment
...
`rune-decompose` reconciliation. The reconciler alone writes the immutable root-cause task
...
`drafting`, validate `rune-taskfmt`'s exact Vision-document headings, per-topic status,
...
summarize its apparent settled content to the user once, and route to `rune-vision` to confirm
...
Per `rune-ledger`. The critical step, and the one that is easy to skip because
...
- neither exists, but the paired worker has a durable non-report outcome → mark the slot
  `unused`
- neither exists and a standalone `rune-drift` record-only, `rune-investigate`, or
  `rune-research` dispatch can be reconstructed entirely from durable pointers → re-dispatch
...
evidence, use `rune-drift` quiesce for unpublished registered worktrees, and use `rune-land`
...
so never send one to `rune-execute` or `rune-recover`.
...
- A progress file ending in `diagnosis: reproduced` with valid diagnosis commit ids means
  ...
  route `rune-work` back into the same decomposition run.
...
- No terminal diagnosis block means the diagnosis worker died. Re-dispatch `rune-bug` with
...
1. **Valid complete publication present?** The executor committed the task and appended
   ...
   `v`, and set `resume_at: verify`; then dispatch `rune-verify` with that attempt and the
...
3. **No handoff?** The session died mid-flight. At the ledger's exact absolute
   ...
   - empty diff, branch not ahead → dispatch `rune-drift` in `discard-empty` mode with the
   ...
   - non-empty → work exists but is unexplained. **Dispatch `rune-recover`** with the same
     ...
     and dispatch `rune-drift` in record-only mode from the task and recovery handoff. Promote
...
- orphaned worktrees with no ledger row → dispatch one `rune-land` worker in `cleanup` mode
...
- `verifying` rows whose verifier never returned → first check for a durable block matching
  ...
  apply the freeze transition, or re-dispatch `rune-drift` with the same id and paths when
  both files are absent. Never allocate a second DRF for the same verdict. Consume every
  other verdict with `rune-work`'s complete mapping (including `failures++` only for `fail`, the finding
...
- `landing` rows whose lander never returned → first consume a durable block matching the
  current `l` with `rune-work`'s complete outcome mapping when present. Otherwise, if `l` is
...
- decomposition runs with a protocol record or planner drafts but no registered tasks →
  ...
  route back to `rune-work`. It allocates a fresh `R-nnn`; never resume into or reuse the
...
- drift-replan runs with new task files or `replacements.md` but no atomic ledger
  ...
  artifact and burn every file id, leave the old rows drift-blocked, and route `rune-work`
...
- `unsized` rows → the task file exists but no fresh worker has said one executor could
  ...
  attempt, means the sizer died: redispatch `rune-size` for that one task after proving the
...
- mitigation rows → for canonical tasks, validate `root_cause_followup` against a
  ...
  outputs is redispatched to `rune-decompose` with the same `work`, reserved root id, and
...
- **staged worker questions** → collect valid `decisions/open/T-nnn-eN.md` files, require
  ...
  `rune-taskfmt`'s decision-allocation transaction. If no decision has that unique
...
- **unverified findings** → collect `findings/open/T-nnn-eN-K.md` claims and sort them by
  ...
  `rune-verify-finding` with the same id and paths, after proving the prior worker stopped.
...
Follow `rune-report`. Say what was **repaired**, not just what exists — silent repair looks
