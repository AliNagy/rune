...
Milestone-graph work from `vision` is the only job here without a protocol pointer;
...
3. Use `serena` to look at the actual symbols the milestone touches in the correct
...
**Milestone graph** (from `vision`) — read `vision.md`, `decisions.md`, and where they
...
**Planner draft** (from `work`) — the work id names one assigned slot such as
...
then write one complete candidate cut there per `taskfmt`. Repeat the protocol `type`
...
**Completed mitigation repair** (from `continue`) — the work id names the fresh run
...
exact assigned `mitigation-repair.md` artifact from `taskfmt`; never edit the old task.
...
**Reconcile** (from `work`) — the work id names one run such as `M-03/R-002`, and you
...
`taskfmt`. Map every protocol `retiring` id exactly once to `none` or one or more new
...
- **Bug / `bug`:** the task is grounded in the already-observed reproduction; the
...
- **Feature / `feature`:** the scope boundary and exclusions are present; every
  user-visible decision is settled; tasks are vertical slices; and each task names its
  integration point and failure behavior.
- **Refactor / `refactor`:** the characterization net exists or is established first;
...
and performs the one ledger transaction defined by `ledger`: add the new rows and mark
