...
- **Create** the canonical empty schema-2 ledger only for the documented empty-project
  bootstrap, then **write** only its top-level `vision` phase using `rune-ledger`; preserve
...
`rune-root` may run only its own separately bounded migration probe while this route follows
...
- **Missing, and the repo is empty** → new project. Vision runs first; init runs after the
  ...
  canonical empty schema-2 shape from `rune-init` with `oracle: —`, `main: green`, and
  `vision: absent` immediately after `rune-root` scaffolds the coordination root. Validate
...
After the ground is ready, validate schema 2 per `rune-ledger`. Before asking the first
...
Write `vision.md` incrementally using `rune-taskfmt`'s exact **Vision document** routing
...
1. **Survey** — dispatch a subagent that follows `rune-survey`, carrying `work: survey`,
   `main_root`, and
...
`<main_root>/.rune/decisions.md`, per `rune-taskfmt`:
...
before it, `rune-continue` may validate the same checklist and finish only this transition; if
...
**Dispatch a subagent that follows `rune-decompose` with `work: vision/graph`, `main_root`,
...
`rune-taskfmt`, and returns the list in ≤200 tokens for you to show the user.
