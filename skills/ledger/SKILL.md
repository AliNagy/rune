...
`taskfmt`. Workers never touch it; they report, the parent records.
...
itself to `land`. Where this file used to
...
`taskfmt`, including while that parent follows `init`, `vision`, or `continue`.
...
interrupted, `continue` validates `taskfmt`'s exact Vision-document checklist and may perform only the
...
`commands | oracle | init/commands | oracle: <command>`. `vision` creates that canonical bootstrap
...
For pre-field immutable tasks, use `taskfmt`'s deterministic reader normalization.
...
`taskfmt`'s repair overlay: exactly one `mitigation-repair` dispatch row must be
...
write. Validation is an agent obligation at the `ledger` interface; Rune does not ship
...
- reconciled task files with no `decompose` planner and reconciler rows → the parent
...
- `diagnosing` — a bug id and worktree are reserved, but its immutable task spec does not
  exist yet. Only `bug` may hold it; it is never executable.
- `unsized` — the task file exists and is registered, but no fresh worker has yet said one
  ...
  cannot be claimed, and no route needs to remember to check a flag. Only `size` moves
...
done only after `verify` names the published commit and `land` lands that same SHA.
...
Before any outcome mapping, validate `taskfmt`'s common return envelope. `work` must
...
**The landing procedure lives in `work`; the merge itself lives in `land`.** Here, only
...
`not_landed` is accepted only from `land` `drift-observe` for a task already named in a
`quiescing` drift entry. It never maps to `pending`: preserve the landing finding while an
`drift` quiesce dispatch discards the registered worktree, then move the row to the
...
to unwind the ones that did. What does **not** stay is the merge that failed: `land`
...
**The procedure lives in `continue`.** It dispatches `recover` and hands you a verdict.
...
| no terminal diagnosis block | re-dispatch `bug` with the same task, protocol, progress, and worktree pointers |

Never send a diagnosing reservation to `execute` or `recover`. The former requires
...
| `recover` → `salvage` | `pending`, returned `step:N`, finding points at handoff | kept |
| `recover` → `partial` | `pending`, returned `evidence:<mode>`, finding points at handoff | kept |
| `recover` → `discard` | `pending`, resume `fresh`, finding points at handoff | dropped |
...
a fresh DRF report slot, dispatch `drift` record-only from the task and recovery
...
rolled back, so the main tree is in a state nobody recorded. Re-dispatch `land` on that
...
2. In the same update, set inactive affected rows to `drifted` with
   ...
   registered until an `drift` quiesce dispatch proves it was discarded.
3. Drain active rows without using their normal successor transitions. A returned
   ...
   result. If that return was lost, `land` in `drift-observe` mode may only recognize an
...
`continue` and `work` read the ledger to answer: what is done, what is
