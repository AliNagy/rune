...
record** per `rune-taskfmt`, and blocks final reconciliation and task generation until
resolved.
...
Per `rune-decompose`: ≤5 files, one subsystem, one checkable outcome, each with a
...
- The behaviour half-exists and is broken → `rune-bug`.
- It exists and works, but the shape is wrong → `rune-refactor`.
