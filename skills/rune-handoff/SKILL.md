...
- **When draining in-flight work**, use `rune-pause`'s exact ledger-settlement and assigned
...
`none` — a drain dispatches verification and landing exactly as `rune-pause` does; handoff
...
If work is in flight, drain it exactly as `rune-pause` does — let executors finish, promote any
...
If the user wants out immediately, a running worker still cannot be interrupted — `rune-pause`
says why. Either wait for it to return, or follow `rune-pause` in abandon mode to discard that
...
|---|---|
| a convention the user corrected you on | **dispatch `rune-survey` in `amend` mode** — one convention → `map.md` |
| a codebase gotcha you discovered | **dispatch `rune-survey` in `amend` mode** — one gotcha → a Serena memory |
...
Per `rune-report`. TL;DR, then the block.
