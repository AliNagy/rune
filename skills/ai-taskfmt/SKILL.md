...
Every `D-nnn` repeats the complete final task contract: title, type, verification mode,
local dependencies, goal, context contract, change surface, steps, check, and acceptance.
`Cut notes` records the assumptions, exclusions, and disputed seams the user gate and
