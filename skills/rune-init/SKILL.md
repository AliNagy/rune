...
- **Follow** `rune-root`; its root rename, migration marker, pointer rewrites, and exact
...
`rune-root`, and Git task lifecycle belongs to task-bound workers.
...
Then follow `rune-root` with `work: coordination-root`, the absolute `main_root`, and
...
after that commit exists. A decline stops init before `rune-root`.
...
Record whether it worked. If Serena is unavailable, note it — `rune-serena` fallbacks
apply and the effective budget per task drops considerably.

### 3. Survey

Dispatch a subagent that follows `rune-survey` with `work: survey`, `main_root`, and absolute
output pointers.
...
or unsuccessful, record Serena as degraded and continue with `rune-serena` fallbacks.

### 4. Commands and the oracle

**Dispatch a subagent that follows `rune-oracle`. You do not run these commands
...
The rule it enforces on your behalf, per `rune-oracle`: **run it, do not infer it.** Then
...
transient `rune-oracle` verdicts without translating `passing` to `ok` or `failing` to
...
Ensure every entry in `rune-root`'s authoritative `rune-directory-manifest` exists. This is
...
init result rather than leaving placeholders). For a new project whose `rune-vision` route
...
Validate the complete candidate per `rune-ledger` before writing it. On a re-run, validate
...
one of `rune-ledger`'s exact coordination-only pre-init graph/survey/commands shapes. Replace
...
`rune-continue` for migration, and init never resets task history. Run and
planner-specific directories beneath `drafts/` are created only when `rune-work` assigns a new
...
Follow `rune-report`. TL;DR first, plain words, detail stays on disk.
