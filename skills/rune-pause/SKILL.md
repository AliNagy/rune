...
- **Write** the parent-assigned result of a drained worker question to
  ...
  `<main_root>/.rune/decisions/open/T-nnn-eN.md` staging file, using `rune-taskfmt`'s exact
...
`none` — verification and landing are dispatched; `rune-drift` abandon mode owns discard of
...
follow `rune-root` with `work: coordination-root`, that absolute root, and
...
set *before* the ledger is even read — step 1 below — and so `rune-work`'s precondition is one
...
exactly as `rune-work` and `rune-continue` are. There is no second agent here, only the same main
...
Validate schema 2 before draining and validate every complete replacement per `rune-ledger`.
...
3. Apply the mode:
   - **drain** — wait for each in-flight worker. A completed `rune-bug` diagnosis leaves its
     ...
     slot exactly as `rune-work` does: non-drift outcomes mark it unused; a drifted outcome is
     recorded only after its assigned staging file is atomically promoted, and it never
     advances to verification. A `question` outcome follows `rune-taskfmt`'s deterministic
     ...
     increment `v`, and **dispatch `rune-verify`** against each ledger-recorded
     `worktree_path`; after a pass, set `landing`, increment `l`, and **dispatch `rune-land`**
   ...
   - **abandon** — after confirmation and after each active worker is confirmed stopped,
     dispatch one `rune-drift` worker in `abandon` mode for each exact ledger-recorded
...
4. Settle report slots only after the paired worker is confirmed stopped. Promote and
   ...
   `rune-continue`; never recycle its id or discard its evidence during `abandon`.
5. Confirm no worker is live and no task is left `in_progress` or `landing`. A completed
   ...
   be validated keeps the tree non-clean and routes recovery through `rune-continue`; do not
...
Follow `rune-report`. The reader is about to walk away, so tell them what they are walking
away from.
