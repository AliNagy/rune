...
**Always run as a subagent.** The caller — `rune-init`, `rune-vision`, or `rune-handoff` — must
...
3. **Module map** — for each top-level module: purpose in one line, its entry symbols,
   what it depends on. Use `get_symbols_overview`, not `Read` (see `rune-serena`).
...
`rune-handoff` learns things from the conversation that belong in the map — a convention the
