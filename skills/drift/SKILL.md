...
- **record-only** receives the same assigned id, staging and final paths, one or more
  ...
  `verify` verdict says the task's evidence or acceptance contract is not checkable, or
...
- **quiesce** receives an existing causal drift pointer, task id, and the ledger's exact
  ...
  and return `status: refused`; a `landing` task must be reconciled by `land` instead.
...
**2. Write the handoff note** — `<main_root>/.rune/notes/T-nnn.md`, per `taskfmt`.
