...
Read-only. `serena` applies with full force — this protocol is where exploration
...
- the reserved `RES-nnn` id and paths: load `research`, follow it for that part, and
...
If the user wants them built, that is a separate `work` invocation which will
triage them properly — probably into `feature` or `refactor`, with the
