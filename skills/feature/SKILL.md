...
record** per `taskfmt`, and blocks final reconciliation and task generation until
resolved.
...
Per `decompose`: ≤5 files, one subsystem, one checkable outcome, each with a
...
- The behaviour half-exists and is broken → `bug`.
- It exists and works, but the shape is wrong → `refactor`.
- You cannot scope it because nobody knows what is wanted → stop. That is a vision gap,
  not a feature. Route it back to `vision`.
