...
`rune-continue` handles the easy cases mechanically: a handoff note exists, or the worktree is
...
7. **Write the missing handoff** to `<main_root>/.rune/notes/T-nnn.md`, in the format
   `rune-taskfmt` specifies, so the next executor gets what it should have had. Its
...
oracle passing before the refactor is reapplied. `rune-verify` correctly refuses either mode
...
fresh report slot and dispatches `rune-drift` in record-only mode from those durable pointers.
