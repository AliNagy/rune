...
`drift` quiesce. When the commit is already reachable, the ordinary oracle and cleanup
...
**5. Run the project oracle** in the main tree, per `oracle`. Compare against the
...
the worktree `verify` already ran the oracle in, so a re-run cannot say anything new.
...
in `taskfmt` without anyone having to think about it. It is **separate from the
