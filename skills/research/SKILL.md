...
- **Question** — in a shape that admits an answer, per `investigate` §1.
...
Breadth before depth, exactly as in `survey`. One query formulation finds one
...
behind that decision — recorded by whoever owns that file, per `taskfmt`, not by you.
If it says the plan is wrong, that is `drift`. If the user wants the recommendation
built, that is a separate `work` invocation which will triage it properly.

The gap between "here is what the evidence says" and "I have begun acting on it" is the
point of this protocol, exactly as in `investigate`. Do not close it on your own
initiative.
