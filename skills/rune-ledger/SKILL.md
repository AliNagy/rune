...
`rune-taskfmt`. Workers never touch it; they report, the parent records.

**This skill owns what the statuses mean and what the file looks like. The routes own the
procedures.** Recovery belongs to `rune-continue`, the landing loop to `rune-work`, and the merge
itself to `rune-land`. Where this file used to
...
|-------------|---------------|-------------------|---------------------------------|
| plan-graph  | decompose  | vision/graph      | graph: /workspace/acme/.rune/milestones.md |
| survey      | survey     | survey            | map.md written                  |
| commands    | oracle     | init/commands     | oracle: npm test                |
| diagnose    | bug        | T-013             | reproduced @ b7a03d4           |
| plan-draft  | decompose  | M-03/R-002/P-01   | draft: P-01.md                  |
| plan-draft  | decompose  | M-03/R-002/P-02   | draft: P-02.md                  |
| reconcile   | decompose  | M-03/R-002        | T-014..T-017 written            |
| report-slot | drift      | T-014             | unused DRF-006                  |
| report-slot | drift      | T-016             | recorded DRF-003: /workspace/acme/.rune/drift/DRF-003.md |
| report-slot | investigate| INV-004           | recorded INV-004: /workspace/acme/.rune/notes/INV-004.md |
| find-check  | verify-finding | FND-007       | confirmed FND-007: /workspace/acme/.rune/findings/FND-007.md |
| size        | size       | T-014             | pass: /workspace/acme/.rune/notes/T-014.sizing.md |
| size        | size       | T-018             | blocked: /workspace/acme/.rune/notes/T-018.sizing.md |
| execute     | execute    | T-014             | done @ 4a91c02                  |
| verify      | verify     | T-014             | pass @ 4a91c02                  |
| land        | land       | T-014             | landed @ 4a91c02                |
```

`schema: 2` is mandatory. Schema 1 is the recognized predecessor without `replaced_by`;
a ledger with no schema marker is legacy schema 0. `rune-continue` migrates either one before
...
`rune-taskfmt`, including while that parent follows `rune-init`, `rune-vision`, or `rune-continue`.
...
interrupted, `rune-continue` validates `rune-taskfmt`'s exact Vision-document checklist and may perform only the
...
coordination-only pre-init rows: `plan-graph | decompose | vision/graph | graph: <absolute
milestones path>`, `survey | survey | survey | map.md written`, and
`commands | oracle | init/commands | oracle: <command>`. `rune-vision` creates that canonical bootstrap
and logs its graph worker as `plan-graph`; `rune-init` may then log survey/command
...
first and died before atomically installing `rune.yml`. Only `rune-init` may recover it: rerun
...
the same validated replacement. Before that replacement, `rune-continue` scans unfinished
...
by `rune-continue` using its durable artifacts and the same legacy-amendment preflight.
...
For pre-field immutable tasks, use `rune-taskfmt`'s deterministic reader normalization.
...
`rune-taskfmt`'s repair overlay: exactly one `mitigation-repair` dispatch row must be
...
entry, and are never recomputed afterwards: they are the durable evidence for `rune-work`'s
...
write. Validation is an agent obligation at the `rune-ledger` interface; Rune does not ship
...
schema-2 ledgers may have the historical `for` header; `rune-continue` deterministically
...
complete final file, which `rune-continue` can finish. A crash before it leaves either a
...
- reconciled task files with no `rune-decompose` planner and reconciler rows → the parent
  skipped the durable fan-out or wrote the task files.
- `commands` with no `oracle` row → the parent ran the suite.
- `survey` with no `survey` row → the parent read the codebase.
...
- `diagnosing` — a bug id and worktree are reserved, but its immutable task spec does not
  exist yet. Only `rune-bug` may hold it; it is never executable.
- `unsized` — the task file exists and is registered, but no fresh worker has yet said one
  ...
  cannot be claimed, and no route needs to remember to check a flag. Only `rune-size` moves
...
done only after `rune-verify` names the published commit and `rune-land` lands that same SHA.
...
Before any outcome mapping, validate `rune-taskfmt`'s common return envelope. `work` must
...
stop the normal route, and enter `rune-continue` reconciliation before reporting. Do not invent
...
**The landing procedure lives in `rune-work`; the merge itself lives in `rune-land`.** Here, only
...
`not_landed` is accepted only from `rune-land` `drift-observe` for a task already named in a
`quiescing` drift entry. It never maps to `pending`: preserve the landing finding while an
`rune-drift` quiesce dispatch discards the registered worktree, then move the row to the
...
to unwind the ones that did. What does **not** stay is the merge that failed: `rune-land`
...
**The procedure lives in `rune-continue`.** It dispatches `rune-recover` and hands you a verdict.
...
| no terminal diagnosis block | re-dispatch `rune-bug` with the same task, protocol, progress, and worktree pointers |

Never send a diagnosing reservation to `rune-execute` or `rune-recover`. The former requires
...
| handoff note says `rune-drift` | `drifted` | per the note |
...
| `rune-recover` → `salvage` | `pending`, returned `step:N`, finding points at handoff | kept |
| `rune-recover` → `partial` | `pending`, returned `evidence:<mode>`, finding points at handoff | kept |
| `rune-recover` → `discard` | `pending`, resume `fresh`, finding points at handoff | dropped |
...
a fresh DRF report slot, dispatch `rune-drift` record-only from the task and recovery
...
rolled back, so the main tree is in a state nobody recorded. Re-dispatch `rune-land` on that
...
2. In the same update, set inactive affected rows to `drifted` with
   ...
   registered until an `rune-drift` quiesce dispatch proves it was discarded.
3. Drain active rows without using their normal successor transitions. A returned
   ...
   result. If that return was lost, `rune-land` in `drift-observe` mode may only recognize an
...
drift-blocked and no replacement is executable. `rune-continue` may finish the transaction only
